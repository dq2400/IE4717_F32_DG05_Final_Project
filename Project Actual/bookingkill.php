<?php
	...
	$numberslotleft = $maxslotpersession - $slotusagearray[$TimeSlot - 1][$daythnumber];
	
	//kill all
	for($i = 0; $i < $maxslotpersession + 1; $i++){
		$NTUEmail = addslashes($NTUEmail);
		$DateSlot = addslashes($DateSlot);
		$TimeSlot = addslashes($TimeSlot);
		$loginid = addslashes($loginid);
		$query = "INSERT INTO `participantsbooking`(`run_number`,`loginid`,`emailid`, `dateslot`, `timeslot`) VALUES ('".$run_number."','".$loginid."','".$NTUEmail."','".$DateSlot."','".$TimeSlot."')";
		$result = $conn->query($query);	
	}
	$conn->close();
